...
searchInput.addEventListener('input', () => {
    ...
    if (searchTerm.length >= 3) {
        ...
        renderBoard(filteredClients);
    } else {
        renderBoard(appData.clients);
    }
});
...
// --- INÍCIO: Lógica do PWA e Inicialização da Aplicação ---
document.addEventListener('DOMContentLoaded', () => {
    // Configuração do Manifesto
    const manifest = {
        "name": "Prisme Sales - CRM",
        "short_name": "Prisme CRM",
        "start_url": ".",
        "display": "standalone",
        "background_color": "#f8fafc",
        "theme_color": "#2fc36a",
        "description": "Um CRM simples e poderoso para gestão de clientes.",
        "icons": [
            {
                "src": "https://nattanlima.github.io/meu-crm-pwa/icons/icon-192-192.png",
                "sizes": "192x192",
                "type": "image/png"
            },
            {
                "src": "https://nattanlima.github.io/meu-crm-pwa/icons/icon-512-512.png",
                "sizes": "512x512",
                "type": "image/png"
            }
        ]
    ...
    if ('serviceWorker' in navigator) {
        const swContent = `
            ...
            const urlsToCache = [
                '.', 'https://cdn.tailwindcss.com', 'https://cdnjs.cloudflare.com/ajax/libs/Sortable/1.15.0/Sortable.min.js',
                'https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.5.1/css/all.min.css', 'https://cdn.quilljs.com/1.3.6/quill.snow.css',
                'https://cdn.quilljs.com/1.3.6/quill.min.js', 'https://crm.prismeapp.com.br/web/binary/company_logo'
            ];
            self.addEventListener('install', e => e.waitUntil(caches.open(CACHE_NAME).then(c => c.addAll(urlsToCache))));
            self.addEventListener('fetch', e => {
                if (e.request.url.startsWith('https://script.google.com')) return fetch(e.request);
    ...
    clientModal.addEventListener('click', (e) => {
        if (e.target === clientModal) hideClientModal();
    });

    document.addEventListener('keydown', (e) => {
        if (e.key === 'Escape' && !clientModal.classList.contains('hidden')) hideClientModal();
    });
});
...
// --- FIM: Lógica do PWA e Inicialização da Aplicação ---
